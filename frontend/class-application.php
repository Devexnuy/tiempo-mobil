<?php
if (!class_exists('TMOBIL_Application')) {

    /**
     * TMOBIL_Application
     * Main class for managing frontend apps
     */
    class TMOBIL_Application {
        ...
        protected function check_device(){
            if ( ! class_exists( 'TMOBIL_Detect' ) ) {
                require_once(TMOBIL_PLUGIN_PATH.'frontend/class-detect.php');
            }
            $WMobileDetect = new TMOBIL_Detect();
        ...
        {
            //include(WPMP_PRO_PLUGIN_PATH.'frontend/sections/show-rel.php');
        }
        ...
        {
            include(WPMP_PRO_PLUGIN_PATH.'frontend/sections/smart-app-banner.php');
        }
        ...
        {
            include(TMOBIL_PLUGIN_PATH.'frontend/sections/show-mobile-link.php');
        }
        ...
        public static function load_app_settings()
        {
            ...
            foreach ($frontend_options as $option_name){
                $settings[$option_name] = WPMP_Pro_Options::get_setting($option_name);
            }

            $settings['ads_slots'] = array_merge($settings['ads_slots'], WPMP_Pro_Options::$default_ads_slots);

            // check if custom theme exists and the file size is greater than zero
            if ($settings['theme_timestamp'] != ''){

                $custom_theme_path = WPMP_PRO_FILES_UPLOADS_DIR.'theme-'.$settings['theme_timestamp'].'.css';
            ...
            foreach (array('icon', 'logo', 'cover') as $file_type){
                ...
                $file_path = WPMP_Pro_Options::get_setting($file_type);

                if ($file_path == '' || !file_exists(WPMP_PRO_FILES_UPLOADS_DIR.$file_path))
                    $settings[$file_type] = '';
                else
                    $settings[$file_type] = WPMP_PRO_FILES_UPLOADS_URL.$file_path;

            }

            // generate comments token
            if (!class_exists('WPMP_Pro_Tokens')) {
                require_once(WPMP_PRO_PLUGIN_PATH . 'inc/class-wpmp-tokens.php');
            }

            $settings['comments_token'] = WPMP_Pro_Tokens::get_token();
        ...
        {
            $language_file_path = WPMP_PRO_PLUGIN_PATH.'frontend/locales/'.strip_tags($locale).'.json';

            if (!file_exists($language_file_path)) {
                $language_file_path = WPMP_PRO_PLUGIN_PATH."frontend/locales/default.json";
            }
